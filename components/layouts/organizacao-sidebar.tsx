...
export function OrganizacaoSidebar() {
  ...
  return (
    <>
      <MenuMobile
        rotulo="Organização"
        rotuloClasse="bg-brand-green/10 text-brand-green"
        ativoClasse="from-brand-green to-lime-500 shadow-brand-green/20"
        itens={navPermitido.map(
          ({ href, label, icon }): ItemMenuMobile => ({ href, label, icon })
        )}
        rodape={
          <>
            {funcionarioAtivo && (

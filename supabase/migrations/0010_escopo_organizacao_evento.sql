-- ============================================================
-- Longevida Eventos — 0010: Escopo por ORGANIZAÇÃO + EVENTO + MÓDULO
--
-- OBJETIVO (Sessão 2C/2D):
--   Estrutura: USUÁRIO -> ORGANIZAÇÃO -> EVENTO -> MÓDULO/PERMISSÃO
--
--   * `app_eventos.organizacao_id`: ancora cada evento em uma
--     organização (organizacoes). Todas as tabelas ligadas a evento
--     derivam a organização a partir daqui.
--   * `app_funcionario_eventos`: vínculo funcionário(auth) <-> evento,
--     com lista de módulos específica por evento.
--   * `app_funcionarios.usuario_id`: liga o registro administrativo
--     (email/texto) à identidade real de auth (uuid), sem quebrar nada.
--   * Helpers RLS que substituem as regras genéricas da 0009 por
--     regras com escopo de ORGANIZAÇÃO + EVENTO + MÓDULO.
--
-- PRÉ-REQUISITO:
--   Executar a 0009 ANTES desta. As políticas de app_atletas,
--   app_perfis, catálogos globais e app_funcionarios permanecem as da
--  0009 (dono + módulo), e dependem das funções da 0009
--  (app_sou_dono_*, app_pode_esc_inscritos, app_eh_staff, ...).
--
-- SENTIDO DAS PERMISSÕES (documentado aqui, regra oficial):
--   * app_funcionario_eventos.permissoes = '[]' (vazio)
--     => herda as permissões padrão do papel em organizacao_usuarios
--        (espelho de PERMISSOES_POR_PAPEL da UI).
--   * app_funcionario_eventos.permissoes preenchido
--     => usa SOMENTE as permissões específicas daquele vínculo
--        evento/funcionário (a lista substitui o padrão do papel).
--   * Sem NENHUMA linha em app_funcionario_eventos dentro de uma
--     organização => o funcionário vê TODOS os eventos da organização
--     (comportamento atual preservado, ninguém é bloqueado ao migrar).
--   * Com linhas na organização => vê SOMENTE os eventos listados.
--   * 'administrador' da organização vê TODOS os eventos da própria
--     organização e todos os módulos (sem bypass global / sem super
--     admin nesta migration).
--
-- PRESERVAÇÃO / COMPATIBILIDADE (aditiva, idempotente, sem DROP
-- de tabela/coluna, sem rename, sem apagar registros):
--   * `app_eventos.organizacao_id` nasce NULL e é preenchida por
--     backfill (eventos existentes -> organização "Espaço Longevida").
--   * `app_funcionarios.usuario_id` nasce NULL e é preenchida por
--     e-mail (correspondência inequívoca; usuarios.email é UNIQUE).
--   * `app_funcionarios.permissoes` NÃO é alterada.
--   * Nenhum id existente é alterado.
--
-- PENDÊNCIAS DE CÓDIGO (documentadas; NÃO resolvidas aqui):
--   1) eventos-store (lib/mock/eventos-store.tsx) PRECISA passar a
--      gravar `organizacao_id` ao criar novos eventos. Sem isso, um
--      evento novo nasce sem organização e nenhum staff conseguirá
--      editá-lo (app_pode_escrever_modulo_org(NULL, ...) = false).
--      [RESOLVIDO na Sessão 2E: eventos-store grava a organização real
--       do vínculo do usuário autenticado.]
--   2) persistencia.ts faz load-all/write-all. Com RLS por escopo,
--      o carregamento devolve só o subconjunto autorizado e o "delete
--      dos ausentes" será barrado pelo RLS (sem perda de dados, mas
--      com ruído/estado stale até o refresh). O código precisará ser
--      ajustado ANTES ou JUNTO da ativação definitiva do RLS por
--      escopo (gravação pontual, Server Actions).
--      [PARCIALMENTE RESOLVIDO na Sessão 2E: array vazio não apaga
--       tabela e erro RLS 42501 nunca é tratado como sucesso. A
--       gravação pontual/Server Actions permanece pendente.]
--   3) app_patrocinadores.eventos é jsonb (array de ids) e não permite
--      origem confiável por organização. Limitação conhecida do
--      diagnóstico (Sessão 2C); permanece como catálogo visível a
--      staff (política da 0009), sem escopo por evento nesta migration.
--   4) app_funcionarios.organizacao_id é texto ('1') e não casa com o
--      uuid de organizacoes. O escopo de staff/config segue as regras
--      da 0009 (staff lê, admin escreve). Reconciliar esse campo com
--      uuid real fica para uma migração futura de funcionários.
--
-- PENDÊNCIAS FUTURAS (documentadas na Sessão 2G; NÃO implementadas):
--   * Tornar app_eventos.organizacao_id NOT NULL (após garantir que o
--     código sempre grava a org) para impedir eventos órfãos.
--   * Backfill por organização REAL (hoje assume a org demo
--     "Espaço Longevida") quando houver múltiplas organizações.
--   * Migração do modelo atletaNome/atletas (identificação por id).
--   * Estruturar app_patrocinadores com vínculo confiável de
--     organização/evento.
--   * Persistência por gravação pontual / Server Actions.
--   * Índice em app_funcionario_eventos(usuario_id) quando a base
--     crescer (busca principal é por usuário).
-- ============================================================
...
-- organização. NÃO atribui a organização demo a todos os eventos sem
-- organizacao_id (havia duas organizações em produção e o backfill
-- amplo atribuiria o evento errado).
--
-- * bbo2gmwd (2° Aquatlhon - 2026): origem confirmada por evidências
--   contextuais (regulamento com autor "Espaço Longevida", local
--   "Espaço Longevida", leituras de QR por staff da Espaço Longevida).
--   Recebe explicitamente a organização da4938f9-7b9d-41b5-b508-57fa17a5a7cd.
--
-- * hskreu4u (copa natação): evento completamente vazio, sem evidência
--   formal suficiente para determinar a qual organização pertence antes
--   da migração. PERMANECE SEM organizacao_id (NULL) — o backfill não
--   inventa vínculo organizacional.
--
-- * Qualquer outro evento existente ou futuro sem organizacao_id também
--   permanece NULL (comportamento conservador).
--
-- NÃO apaga, não recria, não altera ids. Só preenche onde está vazio.
...
--
-- REGRA DE MÓDULO (aplicada pelos helpers da seção 4):
--   permissoes = []        -> herda PERMISSOES_POR_PAPEL[papel]
--   permissoes preenchido  -> usa só a lista deste vínculo
--
-- Exemplo: funcionário X -> evento A -> ['cronometragem','inscritos']
--   significa: acesso a cronometragem e a leitura QR/check-in
--   (inscritos) no evento A; financeiro NEGADO.
-- ============================================================
...
begin
  ...
  if not exists (select 1 from pg_constraint where conname = 'app_funcionario_eventos_usuario_evento_key') then
    -- Tabela nova nasce vazia; sem risco de duplicata com dados atuais.
    alter table public.app_funcionario_eventos
...
-- Touch automático de updated_at.
...
-- RLS da junção: o usuário vê os PRÓPRIOS vínculos; o administrador da
-- organização do evento gerencia (lê/escreve) os vínculos daquela org.
-- (As policies da junção são criadas na seção 7, DEPOIS dos helpers da
--  seção 4 — o PostgreSQL resolve as funções no momento do CREATE POLICY.
--  Correção de ordem aprovada na Sessão 2I.)
alter table public.app_funcionario_eventos enable row level security;

-- GRANT obrigatório da tabela: no PostgREST a permissão de tabela é
-- verificada ANTES do RLS. Sem este grant, nem o administrador da
-- organização conseguiria ler/gerenciar os vínculos da junção.
-- (Correção apontada na auditoria técnica — Sessão 2G.)
...
-- usuarios.email é UNIQUE, então o casamento é determinístico;
-- funcionários sem usuário correspondente permanecem NULL.
...
--   * set search_path = '' (não depende do search_path do chamador);
--   * tabelas e funções qualificadas com public.* / auth.*;
--   * EXECUTE negado ao papel PUBLIC e concedido a anon/authenticated;
--   * auth.uid()/auth.email() leem o JWT da requisição e funcionam
--     dentro de funções SECURITY DEFINER.
-- ============================================================

-- Papel do usuário autenticado em UMA organização específica.
...
-- Módulos padrão de um papel (espelho de PERMISSOES_POR_PAPEL da UI).
-- 'leitura' mantém os módulos de leitura do papel; a escrita é negada
-- nos helpers de escrita (app_pode_escrever_*), nunca aqui.
...
-- Organização de um evento (via app_eventos.organizacao_id).
...
-- Evento de uma inscrição (cadeia indireta inscricao -> evento).
...
-- Evento de uma prova (cadeia indireta prova -> evento).
...
-- O usuário autenticado está autorizado ao evento?
-- (administrador da org => sempre; sem vínculos na org => todos;
--  com vínculos na org => somente os listados na junção.)
...
-- Conjunto de eventos autorizados do usuário autenticado.
...
-- Módulo permitido no contexto de UM evento (respeita a regra de
-- herança/lista explícita da junção para o evento informado).
...
-- INTERAÇÃO com módulo em um evento (leitura/interação; inclui 'leitura').
...
-- ESCRITA em módulo de um evento ('leitura' nunca escreve).
...
-- ESCRITA em módulo NO NÍVEL DA ORGANIZAÇÃO (usada na própria linha
-- de app_eventos e na junção app_funcionario_eventos, onde o evento
-- ainda não existe / é o próprio registro).
...
-- LEITURA RESTRITA de app_inscricoes para a CRONOMETRAGEM.
--
-- Contexto (auditoria — Sessão 2G): a tela de Cronometragem monta a
-- lista de atletas de uma prova lendo app_inscricoes. O papel
-- 'cronometragem' NÃO possui o módulo 'inscritos' (e não deve ganhá-lo,
-- para não obter escrita no módulo nem acesso a app_atletas/perfis).
--
-- Solução (menor privilégio possível):
--   * Somente o papel 'cronometragem' (na organização do evento) recebe
--     esta leitura — administrador/organizador já leem via 'inscritos'.
--   * Somente eventos AUTORIZADOS ao usuário (app_evento_autorizado).
--   * É uma regra de SELECT pura: a política de escrita de
--     app_inscricoes continua exigindo o módulo 'inscritos', portanto o
--     cronometrista NÃO insere/altera/exclui inscrições.
--   * Expõe somente as colunas de app_inscricoes (id, evento_id,
--     prova_id, atleta_nome, status, data_inscricao, numero_peito) — a
--     tabela não contém PII, endereço nem dados financeiros. Nada de
--     app_atletas, app_perfis, app_pagamentos ou app_qrcodes.
...
--
-- NÃO remove segurança sem substituir por regra equivalente ou mais
-- forte. Políticas de catálogos globais, app_atletas, app_perfis e
-- app_funcionarios (dono + módulo) permanecem as da 0009.
-- ============================================================

-- ---------- 7.0 app_funcionario_eventos: próprio vínculo ou admin da org ----------
-- (Bloco movido da seção 2 para cá na Sessão 2I: usa app_org_do_evento()
--  e app_pode_escrever_modulo_org(), que agora já existem quando o
--  CREATE POLICY roda. Lógica idêntica à versão anterior.)
...
--
-- -- Organizações por evento (nada deve sobrar NULL após o backfill):
-- select count(*) from app_eventos where organizacao_id is null;
--
-- -- Funcionários sem vínculo auth (esperado: os e-mails demo que não
-- -- existem em usuarios ficam NULL):
